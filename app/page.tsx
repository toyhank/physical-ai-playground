...
export default function Home(){
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="simulationPanel"><div className="panelHeading"><div><span className="eyebrow">LIVE SIMULATION</span><h2>Franka Panda · MuJoCo</h2></div><div className="metrics"><span>{sceneState?.control_mode==="actuator-mj_step"?"MJ_STEP":"WEBGL 3D"}</span><span>{sceneState?.physics??"waiting"}</span><span>{sceneState?.actuator_count??0} ACT</span></div></div><div className={`scene ${running?"isRunning":""}`}><ClientRobotScene sceneState={sceneState} running={running}/><div className="sceneLegend"><span><i className="redDot"/> grasp target</span><span><i className="blueDot"/> place target</span><span>drag rotate · wheel zoom · right-drag pan</span></div><div className="cameraTag">FRAME {sceneState?.frame??0} · STEP {sceneState?.simulation_steps??0}</div></div></div>
      <aside className="sideRail"><section className="cameraPanel"><div className="sectionTitle"><div><span className="eyebrow">MODEL VISION</span><h3>Panda wrist camera</h3></div><span className="livePill">{online?"LIVE":"WAIT"}</span></div><div className="cameraFeed">{sessionId&&mode==="backend"?<img src={`${API_URL}/api/sessions/${sessionId}/camera.png?v=${cameraVersion}`} alt="Live RGB view from the Franka Panda wrist camera"/>:<><div className="cameraCube"/><div className="cameraBox"/></>}<div className="crosshair">+</div><span>512 × 512 · eye-in-hand RGB · model view</span></div></section><section className="tracePanel"><div className="sectionTitle"><div><span className="eyebrow">REASONING LOOP</span><h3>Agent trace</h3></div><span className="stepCount">{trace.length} events</span></div><div className="traceList">{trace.map((event,index)=><div className={`traceEvent ${event.tone??""}`} key={`${event.kind}-${index}`}><span className={`traceKind ${event.kind.toLowerCase()}`}>{event.kind}</span><p>{event.text}</p></div>)}{running&&<div className="traceEvent pending"><span className="pulse"/><p>Waiting for the next simulator observation</p></div>}</div></section></aside></section>
