...
export default function Home(){
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="simulationPanel"><div className="panelHeading"><div><span className="eyebrow">LIVE SIMULATION</span><h2>7-axis robot workspace</h2></div><div className="metrics"><span>WEBGL 3D</span><span>{sceneState?.physics??"waiting"}</span></div></div><div className={`scene ${running?"isRunning":""}`}><RobotScene3D sceneState={sceneState} running={running}/><div className="sceneLegend"><span><i className="redDot"/> grasp target</span><span><i className="blueDot"/> place target</span><span>drag to orbit · wheel to zoom</span></div><div className="cameraTag">FRAME {sceneState?.frame??0}</div></div></div>
