...
type SceneState = { frame:number; robot:{joints:number[];ee_position:number[];gripper_open:boolean}; objects:{name:string;position:number[]}[]; grasped:boolean; verified:boolean; physics:string };
...
const browserDemoState: SceneState = {
  frame: 0,
  robot: {joints:[0,0,0,0,0,0,0],ee_position:[0,0,0.7],gripper_open:true},
...
function objectStyle(position:number[]|undefined):CSSProperties { return position ? {left:`${49+position[0]*82}%`,top:`${64-position[1]*90}%`} : {}; }
...
export default function Home(){
  ...
  const objects=useMemo(()=>Object.fromEntries((sceneState?.objects??[]).map(item=>[item.name,item.position])),[sceneState]);
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="simulationPanel"><div className="panelHeading"><div><span className="eyebrow">LIVE SIMULATION</span><h2>Franka Panda workspace</h2></div><div className="metrics"><span>20 Hz</span><span>{sceneState?.physics??"waiting"}</span></div></div><div className={`scene ${running?"isRunning":""}`}><div className="sceneGrid"/><div className="tableTop"><div className="redCube dynamicObject" style={objectStyle(objects.red_cube)}><span>RED CUBE</span></div><div className="blueBox dynamicObject" style={objectStyle(objects.blue_box)}><span>BLUE BOX</span></div><div className="robotBase"><div className="joint jointOne"><div className="arm armOne"><div className="joint jointTwo"><div className="arm armTwo"><div className="joint jointThree"><div className={`gripper ${sceneState?.robot.gripper_open?"open":"closed"}`}><b/><b/></div></div></div></div></div></div></div></div><div className="sceneLegend"><span><i className="redDot"/> grasp target</span><span><i className="blueDot"/> place target</span></div><div className="cameraTag">FRAME {sceneState?.frame??0}</div></div></div>
