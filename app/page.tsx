...
type SceneState = { frame:number; robot:{joints:number[];ee_position:number[];gripper_open:boolean}; objects:{name:string;position:number[]}[]; grasped:boolean; verified:boolean; physics:string; control_mode?:string; simulation_steps?:number; actuator_count?:number };
...
export default function Home(){
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="simulationPanel"><div className="panelHeading"><div><span className="eyebrow">LIVE SIMULATION</span><h2>7-axis robot workspace</h2></div><div className="metrics"><span>{sceneState?.control_mode==="actuator-mj_step"?"MJ_STEP":"WEBGL 3D"}</span><span>{sceneState?.physics??"waiting"}</span><span>{sceneState?.actuator_count??0} ACT</span></div></div><div className={`scene ${running?"isRunning":""}`}><ClientRobotScene sceneState={sceneState} running={running}/><div className="sceneLegend"><span><i className="redDot"/> grasp target</span><span><i className="blueDot"/> place target</span><span>drag to orbit · wheel to zoom</span></div><div className="cameraTag">FRAME {sceneState?.frame??0} · STEP {sceneState?.simulation_steps??0}</div></div></div>
