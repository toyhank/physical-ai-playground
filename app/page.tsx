...
type SceneState = { frame:number; robot:{joints:number[];finger_joints?:number[];bodies?:{name:string;matrix:number[]}[];ee_position:number[];gripper_open:boolean}; objects:{name:string;position:number[]}[]; grasped:boolean; verified:boolean; physics:string; control_mode?:string; simulation_steps?:number; actuator_count?:number };
...
const browserDemoState: SceneState = {
  frame: 0,
  robot: {joints:[0,-0.785,0,-2.356,0,1.571,0.785],finger_joints:[0.04,0.04],ee_position:[0.307,-0.37,0.927],gripper_open:true},
  objects: [
    {name:"red_cube",position:[-0.24,0.02,0.47]},
    {name:"green_cube",position:[-0.08,0.17,0.47]},
    {name:"yellow_cube",position:[-0.31,0.26,0.47]},
    {name:"purple_cube",position:[0.04,0.29,0.47]},
    {name:"blue_box",position:[0.25,0.11,0.452]},
  ],
  grasped:false,
  verified:false,
...
const browserPlan: TraceEvent[] = [
  {kind:"OBSERVE",text:"Captured fixed scene + wrist cameras"},
  {kind:"MODEL",text:"Identified red_cube and blue_box"},
  {kind:"TOOL",text:'pick_object({"object_id":"red_cube"})'},
  {kind:"RESULT",text:"verified grasp skill completed",tone:"good"},
  {kind:"TOOL",text:'place_object({"container_id":"blue_box"})'},
  {kind:"RESULT",text:"verified place skill completed",tone:"good"},
...
function ClientRobotScene({sceneState,running}:{sceneState:SceneState|null;running:boolean}){
  ...
  return <Suspense fallback={<div className="threeLoading">Loading Franka Panda meshes…</div>}><RobotScene3D sceneState={sceneState} running={running}/></Suspense>;
}
...
export default function Home(){
  ...
  const [provider,setProvider]=useState<"mock"|"gemini"|"browser">("mock");
  ...
  useEffect(()=>{let cancelled=false; async function connect(){try{const response=await fetch(`${API_URL}/api/sessions`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({seed:Math.floor(Math.random()*10000)})});if(!response.ok)throw new Error(`Backend returned ${response.status}`);const payload=await response.json();if(cancelled)return;const activeProvider=payload.model_provider==="gemini"?"gemini":"mock";setProvider(activeProvider);setSessionId(payload.session_id);setSceneState(payload.state);setOnline(true);setMode("backend");setTrace([{kind:"SYSTEM",text:`Simulation ready · ${payload.state.physics}`},{kind:"OBSERVE",text:activeProvider==="gemini"?"Gemini sees fixed scene + Panda wrist RGB":"Mock planner uses the same semantic skill layer"}]);const socket=new WebSocket(`${API_URL.replace(/^http/,"ws")}/ws/${payload.session_id}`);socketRef.current=socket;socket.onmessage=(message)=>{const event=JSON.parse(message.data) as StreamEvent;if(event.type==="scene_state"&&event.state){setSceneState(event.state);if(event.state.frame%3===0)setCameraVersion(value=>value+1)}const item=describeEvent(event);if(item)setTrace(items=>[...items,item]);if(event.type==="observe")setCameraVersion(value=>value+1);if(["success","failure","stopped"].includes(event.type))setRunning(false)};socket.onclose=()=>setOnline(false)}catch{if(cancelled)return;setProvider("browser");setSessionId("browser-demo");setSceneState(browserDemoState);setOnline(true);setMode("browser");setTrace([{kind:"SYSTEM",text:"Interactive browser demo ready · no API key required"},{kind:"OBSERVE",text:"Connect the Python backend for live MuJoCo physics"}])}}connect();return()=>{cancelled=true;demoToken.current+=1;socketRef.current?.close()}},[]);
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="simulationPanel"><div className="panelHeading"><div><span className="eyebrow">LIVE SIMULATION</span><h2>Franka Panda · MuJoCo</h2></div><div className="metrics"><span>{sceneState?.control_mode==="actuator-mj_step"?"MJ_STEP":"WEBGL 3D"}</span><span>{sceneState?.physics??"waiting"}</span><span className={provider==="gemini"?"visionMode":"mockMode"}>{provider==="gemini"?"GEMINI VISION":provider==="mock"?"MOCK · GROUND TRUTH":"BROWSER DEMO"}</span><span>{sceneState?.actuator_count??0} ACT</span></div></div><div className={`scene ${running?"isRunning":""}`}><ClientRobotScene sceneState={sceneState} running={running}/><div className="sceneLegend"><span><i className="multiDot"/> choose by color</span><span><i className="blueDot"/> place target</span><span>drag rotate · wheel zoom · right-drag pan</span></div><div className="cameraTag">FRAME {sceneState?.frame??0} · STEP {sceneState?.simulation_steps??0}</div></div></div>
      <aside className="sideRail"><section className="cameraPanel"><div className="sectionTitle"><div><span className="eyebrow">MODEL VISION</span><h3>Global + wrist cameras</h3></div><span className="livePill">{online?"LIVE":"WAIT"}</span></div><div className="cameraGrid"><div className="cameraFeed">{sessionId&&mode==="backend"?<img src={`${API_URL}/api/sessions/${sessionId}/camera.png?view=scene&v=${cameraVersion}`} alt="Fixed global RGB scene"/>:<><div className="cameraCube"/><div className="cameraBox"/></>}<span>GLOBAL · stable world view</span></div><div className="cameraFeed">{sessionId&&mode==="backend"?<img src={`${API_URL}/api/sessions/${sessionId}/camera.png?view=wrist&v=${cameraVersion}`} alt="Live RGB view from the Franka Panda wrist camera"/>:<><div className="cameraCube"/><div className="cameraBox"/></>}<span>WRIST · close-up view</span></div></div></section><section className="tracePanel"><div className="sectionTitle"><div><span className="eyebrow">REASONING LOOP</span><h3>Agent trace</h3></div><span className="stepCount">{trace.length} events</span></div><div className="traceList">{trace.map((event,index)=><div className={`traceEvent ${event.tone??""}`} key={`${event.kind}-${index}`}><span className={`traceKind ${event.kind.toLowerCase()}`}>{event.kind}</span><p>{event.text}</p></div>)}{running&&<div className="traceEvent pending"><span className="pulse"/><p>Waiting for the next simulator observation</p></div>}</div></section></aside></section>
    <section className="commandBar"><div className="commandCopy"><span className="eyebrow">NATURAL LANGUAGE CONTROL</span><label htmlFor="task">Give the robot a task</label></div><div className="commandInput"><input id="task" value={prompt} onChange={event=>setPrompt(event.target.value)} onKeyDown={event=>{if(event.key==="Enter")runDemo()}}/><button className="runButton" type="button" onClick={runDemo} disabled={running||!online}>{running?"Running…":"Run task"}</button><button className="stopButton" type="button" onClick={stopDemo} disabled={!running}>Stop</button></div><p className="hint">试试红色、绿色、黄色或紫色方块 · Final success is verified from simulator ground truth.</p></section>
  </main>
}
